...
def build_daily_summary(state: dict[str, Any], scan_context: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"Price: {scan_context.get('price_thb', 0):,.2f} THB",
            f"BTC Guard: {scan_context.get('btc_guard', {}).get('status', '-')}",
            f"Signal: {scan_context.get('signal_label', '-')}",
            f"Entry Score: {scan_context.get('entry_score', '-')}",
            f"Bounce Probability: {scan_context.get('bounce_probability', '-')}",
            f"Opportunity Score: {scan_context.get('opportunity_score', '-')}",
            "",
            f"Signal Count: {counts}",
            f"Position: {state.get('position', {}).get('total_zec', 0)} ZEC",
